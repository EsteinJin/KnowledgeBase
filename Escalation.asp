    ...
    <ul>
      ...
      if isnumeric(request.querystring("page")) then
		...
		rs.absolutepage = 1
			...
			if len(Summary) > 50 then
			Summary = left(Summary,50)
			Summary = Summary & "..."
			end if				    
	
	  %>
      <li><em>[<%=rs("EscalatedDate")%>]</em> [<%=rs("EscalationType")%>] --[<%=rs("EscalatedBy")%>]-- <a href="EscalationDetail.asp?ShowId=<%=rs("ID")%>"><span><%=Summary%></span></a></li>
      <%
				rs.movenext
			next	
end if 
...
<p style="text-align:center;padding:10px;">
	...
	for i = 1 to rs.pagecount
		response.write "<a href='Escalation.asp?page="&i&"'>" & i & "</a> | "
	next
%>
