...
<%
	...
	'去查询这个类别有没有子类
	set rs = server.createobject("adodb.recordset")
	sql = "select * from CMS_Vote where CMS_VoteSid="&voteId
	rs.open sql,conn,1,1
	...
	if not rs.eof then
		...
		dim delsql
		delsql = "delete from CMS_Vote where CMS_ID="&voteId
		conn.execute(delsql)
		call close_rs
		call close_conn
		call sussLoctionHref("Successfully Deleted!","admin_vote_name.asp")
	end if
